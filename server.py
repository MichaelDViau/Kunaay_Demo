...
def init_db():
    ...
    with get_db() as conn:
        ...
        if "location" not in columns:
            conn.execute(
                "ALTER TABLE listings ADD COLUMN location TEXT NOT NULL DEFAULT ''"
            )
        conn.execute(
            """
    ...
    with get_db() as conn:
        ...
        for item in seed_data:
            ...
            while conn.execute("SELECT 1 FROM listings WHERE slug = ?", (slug,)).fetchone():
                ...
                suffix += 1
            cur = conn.execute(
                "INSERT INTO listings (title, slug, category, summary, description, location, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (item["title"], slug, item["category"], item["summary"], item["description"], item["location"], now, now),
            )
...
class AppHandler(SimpleHTTPRequestHandler):
    ...
    def create_listing(self):
        ...
        location = (form.getfirst("location", "").strip())
        if category not in {"rental", "sale"}:
            self.send_json({"error": "Category must be rental or sale"}, status=HTTPStatus.BAD_REQUEST)
            return
        if not title or not summary or not description or not location:
            self.send_json({"error": "Title, summary, description, and location are required"}, status=HTTPStatus.BAD_REQUEST)
            return
        file_items = []
        ...
        with get_db() as conn:
            ...
            cur = conn.execute(
                "INSERT INTO listings (title, slug, category, summary, description, location, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (title, slug, category, summary, description, location, now, now),
            )
            listing_id = cur.lastrowid
